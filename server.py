    ...
    def handle_client_key_down(self, client_socket: socket.socket, data: str) -> None:
        ...
        if plane_num == 0:  # If the player is white
            if data == pygame.K_LEFT:
                self.game.planes[plane_num].hspeed = -self.hspeed
            if data == pygame.K_RIGHT:
                self.game.planes[plane_num].hspeed = self.hspeed
            if data == pygame.K_UP:
                self.game.planes[plane_num].vspeed = -self.vspeed
        ...
        if data == pygame.K_SPACE:  # Space key is valid for both colors
            if plane_num == 0:
                if time.time() - self.last_shot_white < 0.5:  # Checking if the player can already shoot again
                    return
                else:
                    self.last_shot_white = time.time()
            elif plane_num == 1:
                if time.time() - self.last_shot_black < 0.5:  # Checking if the player can already shoot again
                    return
                else:
    ...
    def handle_client_key_up(self, client_socket: socket.socket, data: str) -> None:
        ...
        if plane_num == 0:
            if data == pygame.K_LEFT or data == pygame.K_RIGHT:
                self.game.planes[plane_num].hspeed = 0
            if data == pygame.K_UP or data == pygame.K_DOWN:
                self.game.planes[plane_num].vspeed = 0
        else:
            if data == pygame.K_a or data == pygame.K_d:
                self.game.planes[plane_num].hspeed = 0
            if data == pygame.K_w or data == pygame.K_s:
                self.game.planes[plane_num].vspeed = 0
